...
import { TEAS_TYPES, type TeasInput, type TeasRecord, type TeasType } from './types';

interface Props {
  initial?: TeasRecord;
  busy?: boolean;
  error?: string | null;
  onSubmit: (input: TeasInput) => void;
  onCancel: () => void;
  onDelete?: () => void;
}
...
/** Log/edit a TEAS record. Score fields show for tests/exams; duration/topics for study sessions. */
export function RecordForm({ initial, busy, error, onSubmit, onCancel, onDelete }: Props) {
  const [type, setType] = useState<TeasType>(initial?.type ?? 'practice-test');
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Field label="Type">
          <Select value={type} onChange={(e) => setType(e.target.value as TeasType)}>
            {TEAS_TYPES.map((t) => (
