...
import { CATEGORIES, type Category, type WhyNursingEntry } from './types';

/** "Why Nursing" living document — a chronological stream of the moments that crystallize Keira's
 *  why. Timeline with per-category styling, add/edit, and an inviting empty state. */
export default function WhyNursingPage() {
  const { user } = useAuth();
  const toast = useToast();
  const canEdit = canSetPrivate(user?.role); // only Keira authors/edits her own "why"

  const [entries, setEntries] = useState<WhyNursingEntry[]>([]);
  ...
  const [editing, setEditing] = useState<WhyNursingEntry | null>(null);
  const [deleting, setDeleting] = useState<WhyNursingEntry | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not load your Why Nursing entries.');
    } finally {
      setLoading(false);
    }
  }, [category, from, to]);
