...
import { createClinical } from './api';
import { canSetPrivate } from './logic';
import type { Clinical, ClinicalInput, Visibility } from './types';
...
export interface LogFormProps {
  ...
  facilities?: string[];
  onCreated?: (entry: Clinical) => void;
  onCancel?: () => void;
}

/**
 * Log-clinical-hours form. The Private toggle is offered only to Keira (student role); the server
...
export function LogForm({ facilities = [], onCreated, onCancel }: LogFormProps) {
  ...
  async function handleSubmit(e: FormEvent): Promise<void> {
    ...
    try {
      ...
      const created = await createClinical(input);
      toast.success('Clinical hours logged.');
      onCreated?.(created);
