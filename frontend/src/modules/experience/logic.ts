// Pure, framework-free helpers for the Experience Hours UI. Kept out of the React components so they
// can be unit-tested in the node environment (the repo has no jsdom; component rendering is not
// unit-tested, the logic is).

import type { ExperienceEntry, Visibility } from './types';
...
export function sortByDateDesc(items: readonly ExperienceEntry[]): ExperienceEntry[] {
...
export function knownFacilities(items: readonly ExperienceEntry[]): string[] {
...
export function knownDepartments(items: readonly ExperienceEntry[]): string[] {
  const set = new Set<string>();
...
export function filterBySearch(items: readonly ExperienceEntry[], query: string): ExperienceEntry[] {
