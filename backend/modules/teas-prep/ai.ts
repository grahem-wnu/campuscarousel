// AI layer for TEAS Prep: study-plan + analyze. Both go through Bedrock (model/inference-profile
...
export const curatedAnalyzer: Analyzer = async ({ summary, targetScore }) => {
  ...
  if (summary.cumulativeStudyHours < 20) recs.push('Log more study sessions — consistent weekly hours move TEAS scores the most.');
...
function buildPlanPrompt(input: PlanInput): string {
  return [
    'You are a TEAS (nursing entrance exam) coach. Build a weekly study plan as STRICT JSON only',
    '(no prose/fences):',
...
function buildAnalyzePrompt(ctx: AnalyzeContext): string {
  return [
    'You are a TEAS coach. Given this progress summary, respond with STRICT JSON only',
