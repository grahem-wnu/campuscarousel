...
import { makeHandlers, type TeasHandlers } from './handlers.js';
...
async function seed(over: Record<string, unknown>): Promise<string> {
  const r = await data.teas.create({ type: 'practice-test', date: '2026-01-01', ...over } as Parameters<Data['teas']['create']>[0]);
  return r.recordId;
}
...
describe('create / list / detail / update / remove', () => {
  ...
  it('detail/update/remove with 404s', async () => {
    ...
    expect(await data.teas.get(id)).toBeNull();
