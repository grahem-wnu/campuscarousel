...
// (static /teas/progress|study-plan|analyze beat /teas/:id) + zod + envelope, as it runs in the
...
describe('router integration', () => {
  it('401s unauthenticated; 404s unknown path', async () => {
    expect((await dispatch(event('GET', '/teas'))).statusCode).toBe(401);
    expect((await dispatch(event('GET', '/nope', { as: keira }))).statusCode).toBe(404);
  });

  it('creates then lists', async () => {
    const created = await dispatch(event('POST', '/teas', { as: keira, body: { type: 'practice-test', date: '2026-03-01', overallScore: 70 } }));
    expect(created.statusCode).toBe(201);
    const list = await dispatch(event('GET', '/teas', { as: keira }));
    expect((parse(list).records as unknown[])).toHaveLength(1);
  });

  it('routes /teas/progress (static beats :id, no AWS/Bedrock)', async () => {
    await dispatch(event('POST', '/teas', { as: keira, body: { type: 'practice-test', date: '2026-03-01', overallScore: 70 } }));
    const res = await dispatch(event('GET', '/teas/progress', { as: keira }));
    expect(res.statusCode).toBe(200);
    expect((parse(res).summary as { attempts: number }).attempts).toBe(1);
  });

  it('routes /teas/study-plan and /teas/analyze to a deterministic curated result (no model id set)', async () => {
    ...
    try {
      await dispatch(event('POST', '/teas', { as: keira, body: { type: 'practice-test', date: '2026-03-01', overallScore: 60, sectionScores: { math: 50 } } }));
      const plan = await dispatch(event('POST', '/teas/study-plan', { as: keira, body: { examDate: '2026-08-01' } }));
      expect(plan.statusCode).toBe(200);
      expect((parse(plan).plan as { source: string }).source).toBe('curated');
      const analyze = await dispatch(event('POST', '/teas/analyze', { as: keira, body: {} }));
  ...
  it('updates then deletes by id', async () => {
    const created = await dispatch(event('POST', '/teas', { as: keira, body: { type: 'practice-test', date: '2026-03-01', overallScore: 60 } }));
    const id = (parse(created) as { recordId: string }).recordId;
    const put = await dispatch(event('PUT', `/teas/${id}`, { as: keira, body: { overallScore: 88 } }));
    expect(parse(put).overallScore).toBe(88);
    expect((await dispatch(event('DELETE', `/teas/${id}`, { as: keira }))).statusCode).toBe(204);
    expect((await dispatch(event('GET', `/teas/${id}`, { as: keira }))).statusCode).toBe(404);
  });
});
