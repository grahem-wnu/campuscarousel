...
describe('router integration', () => {
  it('rejects an unauthenticated request with 401', async () => {
    const res = await dispatch(event('GET', '/why-nursing'));
  ...
  it('creates then lists — and a parent never sees the private entry', async () => {
    const created = await dispatch(
      event('POST', '/why-nursing', { as: keira, body: newEntry({ title: 'Secret', visibility: 'private' }) }),
    );
    expect(created.statusCode).toBe(201);

    await dispatch(event('POST', '/why-nursing', { as: keira, body: newEntry({ date: '2026-03-02', title: 'Public' }) }));

    const asKeira = await dispatch(event('GET', '/why-nursing', { as: keira }));
    expect(parse(asKeira).entries as unknown[]).toHaveLength(2);

    const asKate = await dispatch(event('GET', '/why-nursing', { as: kate }));
  ...
  it('a parent fetching a private entry by id gets a 403 envelope', async () => {
    ...
    const asKate = await dispatch(event('GET', `/why-nursing/${id}`, { as: kate }));
    ...
    const asKeira = await dispatch(event('GET', `/why-nursing/${id}`, { as: keira }));
  ...
  it('forbids a parent from creating a private entry (403)', async () => {
    const res = await dispatch(
      event('POST', '/why-nursing', { as: kate, body: newEntry({ visibility: 'private' }) }),
    );
    expect(res.statusCode).toBe(403);
  });

  it('updates and deletes through the router', async () => {
    const created = await dispatch(event('POST', '/why-nursing', { as: keira, body: newEntry() }));
    const id = (parse(created) as { entryId: string }).entryId;

    const updated = await dispatch(event('PUT', `/why-nursing/${id}`, { as: keira, body: { title: 'Refined' } }));
    expect(updated.statusCode).toBe(200);
    expect((parse(updated) as { title: string }).title).toBe('Refined');

    const removed = await dispatch(event('DELETE', `/why-nursing/${id}`, { as: keira }));
    expect(removed.statusCode).toBe(204);

    const gone = await dispatch(event('GET', `/why-nursing/${id}`, { as: keira }));
    expect(gone.statusCode).toBe(404);
  });

  it('422s an invalid create body through the router', async () => {
    const res = await dispatch(event('POST', '/why-nursing', { as: keira, body: { date: 'nope', title: '' } }));
