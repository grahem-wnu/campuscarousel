...
describe('runReviewJob', () => {
  it('marks the job complete with the review result', async () => {
    ...
    const job = await data.essayReviewJobs.create({ essayId: essay.essayId, content: 'a draft about nursing', status: 'pending' });
  ...
  it('persists a compact lastReview from an AI rubric review', async () => {
    const data = await mk();
    const essay = await seedEssay(data);
    const job = await data.essayReviewJobs.create({ essayId: essay.essayId, content: 'my draft words', status: 'pending' });
  ...
  it('marks the job failed when the reviewer throws', async () => {
    const data = await mk();
    const essay = await seedEssay(data);
    const job = await data.essayReviewJobs.create({ essayId: essay.essayId, content: 'x', status: 'pending' });
  ...
  it('fails cleanly when the referenced essay is gone', async () => {
    const data = await mk();
    const job = await data.essayReviewJobs.create({ essayId: 'ghost', content: 'x', status: 'pending' });
    await runReviewJob(() => data, curated, now, job.jobId);
