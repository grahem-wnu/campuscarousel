...
describe('buildOverview', () => {
  it('derives per-college rows with deadline countdown, essay progress, and TEAS presence', () => {
    const colleges = [
    ...
    const teas: Teas[] = [{ recordId: 't1', type: 'practice-test', date: '2026-05-01', overallScore: 72, createdAt: 'x', updatedAt: 'x' }];

    const rows = buildOverview(colleges, essays, teas, TODAY);
    ...
    expect(iu.hasTeasScore).toBe(true);

    const osu = rows[1]!;
    expect(osu.essays).toEqual({ total: 2, final: 1, statuses: ['final', 'drafting'] });
  });

  it('sorts colleges without deadlines last and reports no TEAS score', () => {
    ...
    expect(rows[0]?.hasTeasScore).toBe(false);
  });
});
