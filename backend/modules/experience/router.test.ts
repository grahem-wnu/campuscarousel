...
describe('router integration', () => {
  it('rejects an unauthenticated request with 401', async () => {
    const res = await dispatch(event('GET', '/experience'));
  ...
  it('creates then lists — and a parent never sees the private entry', async () => {
    const created = await dispatch(
      event('POST', '/experience', { as: keira, body: newEntry({ facility: 'Secret Clinic', visibility: 'private' }) }),
    );
    expect(created.statusCode).toBe(201);
    await dispatch(event('POST', '/experience', { as: keira, body: newEntry({ facility: 'Public Clinic', date: '2026-03-02' }) }));

    const asKeira = await dispatch(event('GET', '/experience', { as: keira }));
    expect(parse(asKeira).entries as unknown[]).toHaveLength(2);

    const asKate = await dispatch(event('GET', '/experience', { as: kate }));
  ...
  it('a parent fetching a private entry by id gets a 403 envelope', async () => {
    const created = await dispatch(
      event('POST', '/experience', { as: keira, body: newEntry({ visibility: 'private' }) }),
    );
    const id = (parse(created) as { entryId: string }).entryId;
    const res = await dispatch(event('GET', `/experience/${id}`, { as: kate }));
    expect(res.statusCode).toBe(403);
    expect((parse(res).error as { code: string }).code).toBe('forbidden');
  });

  it('updates and deletes a family entry', async () => {
    const created = await dispatch(event('POST', '/experience', { as: keira, body: newEntry() }));
    const id = (parse(created) as { entryId: string }).entryId;

    const updated = await dispatch(event('PUT', `/experience/${id}`, { as: keira, body: { hours: 9 } }));
    expect(updated.statusCode).toBe(200);
    expect((parse(updated) as { hours: number }).hours).toBe(9);

    const removed = await dispatch(event('DELETE', `/experience/${id}`, { as: keira }));
    expect(removed.statusCode).toBe(204);
  });

  it('routes /experience/summary to the summary handler (static beats :id)', async () => {
    await dispatch(event('POST', '/experience', { as: keira, body: newEntry({ hours: 4 }) }));
    const res = await dispatch(event('GET', '/experience/summary', { as: keira }));
    expect(res.statusCode).toBe(200);
    expect(parse(res).totalHours).toBe(4);
  });

  it('routes /experience/supervisors to the supervisors handler', async () => {
    await dispatch(event('POST', '/experience', { as: keira, body: newEntry({ supervisorName: 'Dr. X' }) }));
    const res = await dispatch(event('GET', '/experience/supervisors', { as: keira }));
    expect(res.statusCode).toBe(200);
    expect((parse(res).supervisors as unknown[])).toHaveLength(1);
  });

  it('routes /experience/export to the export handler and returns a PDF envelope', async () => {
    await dispatch(event('POST', '/experience', { as: keira, body: newEntry() }));
    const res = await dispatch(event('POST', '/experience/export', { as: keira, body: {} }));
    expect(res.statusCode).toBe(200);
  ...
  it('forbids a parent from creating a private entry (403)', async () => {
    const res = await dispatch(
      event('POST', '/experience', { as: kate, body: newEntry({ visibility: 'private' }) }),
    );
    expect(res.statusCode).toBe(403);
  });
});
