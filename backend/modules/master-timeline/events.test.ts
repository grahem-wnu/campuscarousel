...
import type { Activity, Certification, College, Goal, Scholarship, Teas, Visit } from '../../shared/data/index.js';
...
describe('buildEvents', () => {
  it('aggregates every source into one date-sorted stream', () => {
    const s = sources({
      ...
      teas: [{ recordId: 't', type: 'official-exam', date: '2026-08-15', createdAt: 'x', updatedAt: 'x' } as Teas, { recordId: 't2', type: 'practice-test', date: '2026-07-01', createdAt: 'x', updatedAt: 'x' } as Teas],
    ...
    expect(events.map((e) => e.source)).toEqual(['activity', 'scholarship', 'goal', 'teas', 'visit', 'certification', 'college', 'college']);
    // practice tests are excluded; only official-exam becomes a teas event
    expect(events.filter((e) => e.source === 'teas')).toHaveLength(1);
