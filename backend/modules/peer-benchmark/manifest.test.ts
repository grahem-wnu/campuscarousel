...
// with injected dependencies). They must register the exact same method+path set.
...
describe('routes.manifest ↔ buildRoutes', () => {
  it('register the identical method+path set', () => {
    const built = buildRoutes(makeHandlers(() => ({}) as Data, () => ({}) as BenchmarkResearcher)).map(sig).sort();
