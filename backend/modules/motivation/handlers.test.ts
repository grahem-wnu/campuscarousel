...
import { makeHandlers, type MotivationHandlers } from './handlers.js';
...
async function seedEntry(over: Record<string, unknown> = {}): Promise<string> {
  const e = await data.motivations.create({
    date: '2026-01-10',
    title: 'Seed',
...
describe('create (POST /motivations)', () => {
...
describe('list (GET /motivations) — visibility filtering', () => {
  beforeEach(async () => {
...
describe('detail (GET /motivations/:id)', () => {
...
describe('update (PUT /motivations/:id)', () => {
...
describe('remove (DELETE /motivations/:id)', () => {
  ...
  it('Keira can delete her private entry (204)', async () => {
    ...
    expect(await data.motivations.get(id)).toBeNull();
  });
});
...
describe('AI visibility — private reaches the AI only when keira is the caller', () => {
  ...
  it('the AI set for keira includes private entries', async () => {
    const all = await data.motivations.list();
