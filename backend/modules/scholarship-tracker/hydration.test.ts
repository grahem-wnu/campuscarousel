...
describe('buildHydrationMessage', () => {
  it('builds a typed, tenant-stamped scholarship-hydrate message', () => {
    expect(runWithTenant('fam1', () => runWithStudent('s1', () => buildHydrationMessage('sc1')))).toEqual({
...
describe('makeSqsEnqueuer', () => {
  it('sends a SendMessageCommand with the message body to the configured queue', async () => {
    ...
    await runWithTenant('fam1', () => runWithStudent('s1', () => enqueuer.enqueue('sc9')));
    ...
    expect(JSON.parse(command.input.MessageBody)).toEqual({ type: 'scholarship-hydrate', scholarshipId: 'sc9', tenantId: 'fam1', studentId: 's1' });
  });
