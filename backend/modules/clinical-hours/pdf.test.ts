...
import { renderClinicalPdf, renderTextPdf } from './pdf.js';
...
describe('renderClinicalPdf', () => {
  it('includes the header, totals, and entry rows', () => {
    ...
    expect(pdf).toContain('Clinical Hours Record');
  ...
  it('handles an empty entry set without throwing', () => {
    const pdf = decode(renderClinicalPdf({ generatedAt: '2026-06-06T00:00:00.000Z', entries: [] }));
    expect(pdf).toContain('No clinical hours logged.');
    expect(pdf.startsWith('%PDF-1.4')).toBe(true);
  });
});
