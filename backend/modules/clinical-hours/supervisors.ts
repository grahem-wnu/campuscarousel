// Pure derivation of the supervisor directory for GET /clinical/supervisors. The directory is built
// from the clinical entries themselves (no separate entity) so it always reflects logged hours.
...
export interface SupervisorEntry {
  ...
  facilities: string[];
  /** Total clinical hours supervised. */
  totalHours: number;
...
/**
 * Group visibility-filtered clinical entries by `supervisorName`. Entries without a supervisor name
 * are ignored (they can't appear in a directory). Sorted by total hours desc, then name.
 */
export function buildDirectory(entries: readonly Clinical[]): SupervisorEntry[] {
