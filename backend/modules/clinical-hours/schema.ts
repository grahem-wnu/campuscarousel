// Request validation for the Clinical Hours endpoints. Every handler validates input with one of
...
/**
 * Body for POST /clinical. `.strict()` rejects unknown fields so typos surface as 422.
 * `hours` is required (a clinical-hours entry without hours is meaningless); `visibility` is only
...
/** Body for PUT /clinical/:id — every field optional. */
export const updateSchema = createSchema.partial();

/** Query for GET /clinical — unknown params are ignored (API Gateway may add some). */
...
/** Body for POST /clinical/export — all optional filters plus an optional student name for the header. */
