// Minimal, dependency-free PDF writer for the clinical-hours export. A clinical-hours summary is
...
import type { Clinical } from '../../shared/data/index.js';
...
export interface ClinicalPdfModel {
  studentName?: string;
  generatedAt: string; // ISO timestamp, passed in for determinism/testability
  entries: readonly Clinical[];
}

const fmtHours = (n: number): string => (Number.isInteger(n) ? String(n) : n.toFixed(2));

/** Build the formatted line list for a clinical-hours export, then render it to PDF bytes. */
export function renderClinicalPdf(model: ClinicalPdfModel): Uint8Array {
  const { studentName, generatedAt, entries } = model;
  const summary = summarize(entries);
  const directory = buildDirectory(entries);
  const lines: Line[] = [];

  lines.push({ text: 'Clinical Hours Record', size: 18 });
  ...
  if (entries.length === 0) {
    lines.push({ text: 'No clinical hours logged.', size: 10 });
  }
