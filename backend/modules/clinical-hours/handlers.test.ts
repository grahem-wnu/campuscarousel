...
import { makeHandlers, type ClinicalHandlers } from './handlers.js';
import type { ClinicalSummary } from './summary.js';
...
/** Seed a clinical entry directly via the data layer (bypassing the create handler). */
async function seed(over: Record<string, unknown> = {}): Promise<string> {
  const c = await data.clinical.create({
    date: '2026-01-10',
    facility: 'Memorial Hospital',
    hours: 4,
    visibility: 'family',
    ...over,
  } as Parameters<Data['clinical']['create']>[0]);
  return c.entryId;
}
...
describe('list (GET /clinical) — visibility filtering', () => {
  beforeEach(async () => {
...
describe('detail (GET /clinical/:id)', () => {
...
describe('update (PUT /clinical/:id)', () => {
...
describe('remove (DELETE /clinical/:id)', () => {
  ...
  it('Keira can delete her private entry (204)', async () => {
    ...
    expect(await data.clinical.get(id)).toBeNull();
  });
});

describe('summary (GET /clinical/summary) — visibility filtering', () => {
  beforeEach(async () => {
  ...
  it("Keira's summary includes private hours", async () => {
    ...
    const s = res.body as ClinicalSummary;
...
describe('supervisors (GET /clinical/supervisors) — visibility filtering', () => {
  beforeEach(async () => {
...
describe('export (POST /clinical/export) — visibility filtering', () => {
  beforeEach(async () => {
