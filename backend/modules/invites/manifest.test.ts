...
describe('routes.manifest ↔ buildRoutes', () => {
  ...
  it('expose the admin invite endpoints, all platform-admin gated', () => {
    expect(manifestRoutes.map(sig).sort()).toEqual(
      ['GET /admin/invites', 'POST /admin/invites', 'POST /admin/invites/:code/revoke'].sort(),
    );
    expect(manifestRoutes.every((r) => r.platformAdmin === true)).toBe(true);
  });
});
