// Pure exam progression + readiness logic. Framework- and AWS-free so it unit-tests for real. The
...
/** Neutral default target score for a competitive standardized exam (0-100 scale). Not tied to any
 *  specific program or major; callers may override with the actual exam's target. */
export const DEFAULT_TARGET_SCORE = 78;
...
export function scoredRecords(records: readonly ExamScore[]): ExamScore[] {
  return records
...
export function progression(records: readonly ExamScore[]): ProgressPoint[] {
...
export function latest(records: readonly ExamScore[]): ExamScore | null {
  const s = scoredRecords(records);
  return s.length ? s[s.length - 1]! : null;
}

/** Best overall score across scored records, or null. */
export function bestOverall(records: readonly ExamScore[]): number | null {
  const scores = scoredRecords(records).map((r) => r.overallScore!).filter((n) => n !== undefined);
  return scores.length ? Math.max(...scores) : null;
}

/** Proficiency band for a section/overall score (generic 0-100 reference points). */
export type Band = 'strong' | 'needs-work' | 'critical';
export function band(score: number): Band {
  if (score >= DEFAULT_TARGET_SCORE) return 'strong'; // ~Advanced
  if (score >= 59) return 'needs-work'; // ~Proficient
  return 'critical';
}

/** Sections of the latest record at or below the needs-work threshold, weakest first. */
export function weakSections(records: readonly ExamScore[], threshold = DEFAULT_TARGET_SCORE): Section[] {
...
export function cumulativeStudyHours(records: readonly ExamScore[]): number {
  return records
...
export function overallTrend(records: readonly ExamScore[]): number | null {
  const s = scoredRecords(records);
...
/** Roll up everything GET /exams/progress returns for the dashboard + charts. */
export function summarize(records: readonly ExamScore[]): ProgressSummary {
  const last = latest(records);
...
/** Simple readiness heuristic against a target score (default = a generic competitive target). */
export function readiness(records: readonly ExamScore[], targetScore = DEFAULT_TARGET_SCORE): {
  ready: boolean;
  gap: number | null;
  message: string;
} {
